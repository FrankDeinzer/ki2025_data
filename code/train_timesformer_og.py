...
# torch.cuda.set_per_process_memory_fraction(0.75)  # Allocate up to 50% of the GPU memory

TRAIN_PATH = '/mnt/herculaneumscrolls/dl.ash2txt.org/full-scrolls/Scroll5/PHerc172.volpkg/paths/'
# LABELS_PATH = 'our_labels/labels'
# LABELS_PATH = 'our_labels/iteration_2/run_2/processed'
LABELS_PATH = 'our_labels/clean_approach/it3/processed'
class CFG:
    # ============== comp exp name =============
    comp_name = 'vesuvius'
    comp_dir_path = './'
...
def read_image_mask(fragment_id,start_idx=17,end_idx=43, CFG=CFG):
    ...
    for i in idxs:
        if os.path.exists(f"{TRAIN_PATH}/{fragment_id}/layers/{i:02}.tif"):
            image = cv2.imread(f"{TRAIN_PATH}/{fragment_id}/layers/{i:02}.tif", 0)
        else:
            image = cv2.imread(f"{TRAIN_PATH}/{fragment_id}/layers/{i:02}.jpg", 0)
    ...
    inklabel_files = glob.glob(f"{LABELS_PATH}/{fragment_id}_mask.png")
    ...
    if os.path.exists(f"{TRAIN_PATH}/{fragment_id}/{fragment_id_}_flat_mask.png"):
        fragment_mask=cv2.imread(f"{TRAIN_PATH}/{fragment_id}/{fragment_id_}_flat_mask.png", 0)
    else:
        fragment_mask=cv2.imread(f"{TRAIN_PATH}/{fragment_id}/{fragment_id_}_mask.png", 0)
...
def worker_function(fragment_id, CFG):
    ...
    if not os.path.exists(f"{TRAIN_PATH}/{fragment_id}"):
...
validation_fragments=['20241028121111']
for fid in validation_fragments:
    CFG.valid_id=fid
    fragment_id = CFG.valid_id
    run_slug=f'training_scrolls_valid={fragment_id}_{CFG.size}x{CFG.size}_submissionlabels'

    valid_mask_gt = cv2.imread(CFG.comp_dataset_path + f"{LABELS_PATH}/{fragment_id}_mask.png", 0)
    ...
    wandb_logger = WandbLogger(project="Vesuvius",name=run_slug+f'timesformer_corrected_labels_connected_it3')
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[ModelCheckpoint(filename=f'connected_it3_timesformer_wild16_{fid}_fr'+'{epoch}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=CFG.epochs),
                    ],
    )
    trainer.fit(model=model, 
                # To resume training from a checkpoint
                # ckpt_path="outputs/vesuvius/pretraining_all/vesuvius-models/connected_it0_timesformer_wild16_20241028121111_frepoch=15.ckpt",
                # ckpt_path="timesformer_weights.ckpt",
